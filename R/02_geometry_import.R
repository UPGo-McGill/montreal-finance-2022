...
DA <-
  cancensus::get_census(
    ...
    vectors = c("v_CA16_4840", "v_CA16_4841", "v_CA16_4836", "v_CA16_4838",
                ...
                "v_CA16_3405", "v_CA16_6698", "v_CA16_6692", "v_CA16_6725", "v_CA16_6719", "v_CA16_4896"),
    geo_format = "sf") %>% 
  st_transform(32618) %>% 
  ...
  set_names(c("dwellings", "GeoUID", "parent_condo", "condo", "parent_tenure", 
              ...
              "parent_mobility_one_year", "mobility_five_years", "parent_mobility_five_years", "geometry")) %>% 
  ...
  select(GeoUID, dwellings, renter, p_thirty_renter, median_rent, average_value_dwellings, p_condo, 
         p_renter, p_repairs, p_vm, p_immigrants, p_mobility_one_year, p_mobility_five_years) %>% 
  as_tibble() %>% 
...
CT <-
  cancensus::get_census(
    ...
    vectors = c("v_CA16_4840", "v_CA16_4841", "v_CA16_4836", "v_CA16_4838",
                ...
                "v_CA16_6719", "v_CA16_4896", "v_CA16_410", "v_CA16_408", "v_CA16_2397",
                "v_CA16_76", "v_CA16_79", "v_CA16_82", "v_CA16_1"),
    geo_format = "sf") %>% 
  st_transform(32618) %>% 
  ...
  set_names(c("GeoUID", "dwellings", "parent_condo", "condo", "parent_tenure", 
              ...
              "five_storeys", "parent_storeys", "med_hh_income", "age_18", "age_19", "age_20_24", "age_total", "geometry")) %>% 
  mutate(p_condo = condo / parent_condo,
         p_renter = renter / parent_tenure, 
         p_repairs = major_repairs / parent_repairs,
         p_vm = vm/parent_vm,
         ...
         p_thirty_renter = p_thirty_renter / 100) %>% 
  select(GeoUID, dwellings, renter, p_thirty_renter, median_rent, average_value_dwellings, p_condo, 
         p_renter, p_repairs, p_vm, p_immigrants, p_mobility_one_year, p_mobility_five_years, p_five_more_storeys, med_hh_income,
         p_18_24) %>% 
  as_tibble() %>% 
  st_as_sf(agr = "constant")
...
CT <-  
  CT %>% 
  mutate(renter = ifelse(is.na(renter)&dwellings==0, 0, renter))
...
CT_06 <- 
  cancensus::get_census(
    ...
    geo_format = "sf") %>% 
  st_transform(32618) %>% 
  select(-c(`Shape Area`, Type, Households, `Quality Flags`, CMA_UID:CSD_UID, PR_UID:`Area (sq km)`)) %>% 
...
CT <- 
  ...
  as_tibble() %>% 
  st_as_sf() %>% 
  mutate(change_renter_dwellings=renter-renter_06) %>% 
  select(-renter, -renter_06) 

#For some reason, the change in renter dwellings gave non-justified NAs. This code fixes that.

CT <-
  CT %>%
  mutate(renter16=p_renter*dwellings) %>%
  left_join(., CT_06 %>% select(GeoUID, renter) %>% st_drop_geometry(), by="GeoUID") %>%
  mutate(change_renter_dwellings = ifelse(is.na(change_renter_dwellings) & !is.na(renter16), renter16-renter, change_renter_dwellings)) %>%
  select(-renter, -renter16)


# Add distance to downtown as a variable -----------------------------------

downtown <- st_point(c(-73.571599, 45.504156))

downtown <- st_sfc(downtown, crs=4326)
...
CT <- 
  CT %>% 
  mutate(distance_dt = st_distance(st_centroid(.), downtown)) 
